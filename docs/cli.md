...
errors. `--root` selects the existing project; no parent-directory project discovery.
...
resolved within `--root`. Source paths are relative to `--root` and may explicitly
name a sibling. Check cwd is confined to its source; report paths are confined to
...
[project templates](../templates/project) remain available. Existing project files
...
Package implementation version and record format version are separate. Receipts in
the supported format remain readable, but a different CLI implementation version is stale.
`current` describes declared local bindings, not all toolchains or remote state.

Applicability covers the entire configuration, task and all declared sources, not
only the selected check's apparent inputs. Any binding change can stale the whole
receipt. `reconcile` does not merge receipts or identify unaffected checks. Preserve
unaffected code, but use a new run of the selected claim for current CLI support;
...
unsupported ones appear as diagnostics. Invalid clinx configuration is an error.
...
`expectedTests` defaults empty, so configure it when test selection matters. Failed
...
project root. Use these to locate a missing executable, bad test selection, failure,
...
steals the lock automatically. Two distinct project roots are not a shared lock domain.
...
a listed task may need revision before `validate`, `context` or `verify` can use it.
...
and attachment capture still require current inputs. Invalid contracts or unreadable
task references must be repaired before context or reconciliation can use them.
