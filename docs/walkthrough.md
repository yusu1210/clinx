...
[中文](walkthrough.zh-CN.md)

This walkthrough uses an existing project, not a scaffolded application. Start with
a built clinx checkout. In commands below `clinx` means your reviewed local binary
(or `node /path/to/clinx/bin/clinx.mjs`); it does not mean an unverified registry package.

Only have a PRD and repository URLs? Begin with [cold start](cold-start.md), without
...
outputs. The [configuration template](../templates/project/clinx.config.json) is a
...
[contract template](../templates/project/clinx/contract.example.json) as a shape example.
...
## 5. Handle a real change of mind
...
optional [map](../templates/project/clinx/system-map.md) or
[guide](../templates/project/clinx/local-guide.md) and current source findings.
