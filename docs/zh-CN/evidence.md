...
`context ID` 和指定任务的 `status ID` 包含 `evidence` 索引，续接时无需预先提供路径即可发现
已存观察。索引保留冲突声明与原有限制，不挑选最新 pass；只读元数据，
`integrity`、`localBinding` 和 `remoteState` 均为 `not-checked`。
复用前通过 `evidence list ID --record UUID` 检查相关归档及当前本地绑定。
本地检查通过不会消除失败观察。

索引最多访问 64 个目录条目，元数据单文件最多 64 KiB、合计读取最多 256 KiB。
超大、损坏、缺失和符号链接记录逐项报告；枚举超过上限时设置 `truncated`。
枚举不按时间排序，并发变化不构成一致快照。部分索引既不是完整历史，也不代表没有失败。
发现过程不读取附件或远端状态、不执行命令，也不改变续接状态或验收声明。
...
使用 `--record UUID` 单独检查。读取已存附件不依赖有效配置、当前约定或源访问；
任一不可用时，`localBinding` 为 unknown 并给出原因，仍核对归档身份与制品完整性。
这不修复缺失输入，也不允许采集新记录；工具不为适应上限而删除旧记录。
...
可运行的[平台观察示例](../../examples/platform-observation/README.zh-CN.md)通过合成 HTTP fixture
验证业务响应、目标/构建对应、终态和用例结果，连接原生测试、附件、续接及源码变化。
真实平台验收仍保持未决。
